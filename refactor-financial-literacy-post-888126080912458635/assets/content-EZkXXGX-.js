import{A as Je}from"./index-Dli2OtB_.js";const mn=`---
type: post
draft: true
status: draft
...
`,Hn=Object.freeze(Object.defineProperty({__proto__:null,default:zn},Symbol.toStringTag,{value:"Module"})),Vn=`---
type: post
...
${t}`)+"'";return e.implicitKey?s:je(s,t,$t,De(e,!1))}function ee(n,e){const{singleQuote:t}=e.options;let s;if(t===!1)s=pe;else{const i=n.includes('"'),o=n.includes("'");i&&!o?s=Ze:o&&!i?s=pe:s=t?Ze:pe}return s(n,e)}let et;try{et=new RegExp(`(^|(?<!
))
+(?!
...
`+p.comment:c.comment=p.comment),c.range=[s.offset,d,p.offset]}else c.range=[s.offset,d,d];return c}function He(n,e,t,s,i,o){const a=t.type==="block-map"?Fs(n,e,t,s,o):t.type==="block-seq"?Ws(n,e,t,s,o):qs(n,e,t,s,o),r=a.constructor;return i==="!"||i===r.tagName?(a.tag=r.tagName,a):(i&&(a.tag=i),a)}function Us(n,e,t,s,i){const o=s.tag,a=o?e.directives.tagName(o.source,h=>i(o,"TAG_RESOLVE_FAILED",h)):null;if(t.type==="block-seq"){const{anchor:h,newlineAfterProp:g}=s,m=h&&o?h.offset>o.offset?h:o:h??o;m&&(!g||g.offset<m.offset)&&i(m,"MISSING_CHAR","Missing newline after block sequence props")}const r=t.type==="block-map"?"map":t.type==="block-seq"?"seq":t.start.source==="{"?"map":"seq";if(!o||!a||a==="!"||a===j.tagName&&r==="map"||a===V.tagName&&r==="seq")return He(n,e,t,i,a);let l=e.schema.tags.find(h=>h.tag===a&&h.collection===r);if(!l){const h=e.schema.knownTags[a];if(h?.collection===r)e.schema.tags.push(Object.assign({},h,{default:!1})),l=h;else return h?i(o,"BAD_COLLECTION_TYPE",`${h.tag} used for ${r} collection, but expects ${h.collection??"scalar"}`,!0):i(o,"TAG_RESOLVE_FAILED",`Unresolved tag: ${a}`,!0),He(n,e,t,i,a)}const c=He(n,e,t,i,a,l),f=l.resolve?.(c,h=>i(o,"TAG_RESOLVE_FAILED",h),e.options)??c,u=O(f)?f:new T(f);return u.range=c.range,u.tag=a,l?.format&&(u.format=l.format),u}function Ks(n,e,t){const s=e.offset,i=Gs(e,n.options.strict,t);if(!i)return{value:"",type:null,comment:"",range:[s,s,s]};const o=i.mode===">"?T.BLOCK_FOLDED:T.BLOCK_LITERAL,a=e.source?zs(e.source):[];let r=a.length;for(let d=a.length-1;d>=0;--d){const p=a[d][1];if(p===""||p==="\r")r=d;else break}if(r===0){const d=i.chomp==="+"&&a.length>0?`
