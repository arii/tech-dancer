...
The goal is to transition `boomtick-pkg` (encompassing the `td` Python tool and the TypeScript MCP server) from a package tailored to `boomtick.blog` (`tech-dancer`) into a highly portable, self-contained utility.
