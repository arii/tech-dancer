...
Calling `python3 -m dev_tools.cli` directly (Tier 2) skips step 1.
...
python3 -m dev_tools.cli gh pr-diff 2280

# ✅ Correct — MCP hydrates .agent-context.json, calls python3 -m dev_tools.cli internally
...
| **Tier 2** | `python3 -m dev_tools.cli` | Only when a Tier 1 tool is unavailable or the CLI provides logic not yet exposed via MCP |
...
| Category | Task | Tier 1: MCP (required first) | Tier 2: `python3 -m dev_tools.cli` (fallback) | Tier 3: Bash (last resort) |
| :--- | :--- | :--- | :--- | :--- |
| **GitHub** | Search PRs | `github.search_open_prs` | `python3 -m dev_tools.cli gh search-prs` | `gh pr list` |
| **GitHub** | Get PR Diff | `github.get_pr_diff` | `python3 -m dev_tools.cli gh pr-diff <PR>` | `gh pr diff <PR>` |
| **GitHub** | Check Conflicts (global) | `github.get_merge_conflict_files` | `python3 -m dev_tools.cli gh conflicts` | - |
| **GitHub** | Detect Conflicts (single PR) | `github.get_merge_conflict_files` | `python3 -m dev_tools.cli gh detect-conflicts --pr <PR>` | `git merge-tree` |
| **GitHub** | Merge Conflicts (PR vs base) | `github.get_merge_conflict_files` | `python3 -m dev_tools.cli gh merge-conflicts <PR>` | - |
| **GitHub** | Resolve Conflicts | `github.create_repair_branch` | `python3 -m dev_tools.cli gh resolve-conflicts --pr <PR>` | - |
| **GitHub** | Comment on PR | `github.comment_triage_summary` | `python3 -m dev_tools.cli gh audit-pr <PR> --submit` | `gh pr comment` |
| **GitHub** | Status Board | `github.get_status_board` | `python3 -m dev_tools.cli gh status-board` | - |
| **GitHub** | PR Overlaps | `github.analyze_overlaps` | `python3 -m dev_tools.cli gh overlaps` | - |
| **GitHub** | Plan Review | `github.plan_review` | `python3 -m dev_tools.cli agent plan-review --pr <PR>` | - |
| **GitHub** | Audit PR (Submit) | `github.audit_pr` | `python3 -m dev_tools.cli gh audit-pr <PR> --submit` | - |
| **GitHub** | Manage Reviews | `github.manage_reviews` | `python3 -m dev_tools.cli gh manage-reviews` | - |
| **GitHub** | Validate Issue | `github.validate_issue` | `python3 -m dev_tools.cli gh validate-issue` | - |
| **GitHub** | Create Issue | `github.create_issue` | `python3 -m dev_tools.cli gh create-issue` | `gh issue create` |
| **GitHub** | Aggregate PRs / Consolidate | - | `python3 -m dev_tools.cli gh aggregate <TARGET_BRANCH> <PR_NUMBERS...>` (Note: Use `main` branch as base) | - |
| **GitHub** | Pre-submit Gate | `github.pre_submit` | `python3 -m dev_tools.cli gh pre-submit` | - |
...
| **Repository** | Read CI Logs | `repo.read_ci_logs` | `python3 -m dev_tools.cli repo ci-logs <PR>` | `gh run view` |
| **Repository** | Runtime Check | `repo.doctor` | `python3 -m dev_tools.cli doctor` | - |
| **Testing** | Run Vitest | `repo.run_tests` | - | `pnpm test` |
| **Testing** | Run Playwright | `repo.run_playwright` | `python3 -m dev_tools.cli repo run-playwright` | `npx playwright test` |
