...
1. **Consult `.agent-context.json`** — submodule synchronization and repository version manifest (`packageName`, `updatedAt`, `gitCommit`, `submodules`, `version`). This file is generated by `boomtick-pkg/cli/dev_tools/resources/build-repo-context.py` and kept fresh by the git hooks in `.githooks/`.
2. **Consult `boomtick-pkg/cli/dev_tools/cli-schema.json`** — canonical authority for local CLI commands (`td-cli`). Dynamic command schema resolution is available via `td-cli schema <path>` or `repo.get_command_schema`. Never guess flags. Never run `--help`.
...
`repo.read_agent_context` provides structured repository context and submodule sync metadata — ensuring deterministic operations without redundant filesystem or git state inspection.
...
### CLI Schema Fallback
If calling Tier 2 directly, query the CLI schema authority (`boomtick-pkg/cli/dev_tools/cli-schema.json`) or use dynamic lookup tools rather than guessing flags or running `--help`:

```bash
# Extract schema for a specific subcommand using td-cli schema or repo.get_command_schema
td-cli schema "gh/pr-diff"

# Or query cli-schema.json directly
cat boomtick-pkg/cli/dev_tools/cli-schema.json | python3 -c "
import json, sys
schema = json.load(sys.stdin)
print(json.dumps(schema['commands']['gh']['subcommands']['pr-diff'], indent=2))
"
```

This is what `boomtick-mcp` handles automatically on tool invocation.
