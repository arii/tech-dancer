...
class GitHubClient:
    ...
    def list_pull_requests(self, state: str = 'open', limit: int = 100, labels: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        if labels:
            # GitHub REST API doesn't support filtering pulls by multiple labels directly in /pulls.
            # We must use search API instead.
            query = f"repo:{self.repo} is:pr state:{state}"
            for label in labels:
                query += f' label:"{label}"'
            params = {"q": query, "per_page": min(limit, 100)}
            search_data = self._request('GET', '/search/issues', params=params)
            prs = []
            for item in search_data.get('items', [])[:limit]:
                # Fetch details for each PR matching the search query to ensure full object mapping
                pr_details = self.fetch_pr_details(item['number'])
                prs.append({
                    "number": pr_details.get("number"),
                    "title": pr_details.get("title"),
                    "author": {"login": pr_details.get("user", {}).get("login")},
                    "headRefName": pr_details.get("head", {}).get("ref"),
                    "baseRefName": pr_details.get("base", {}).get("ref"),
                    "isDraft": pr_details.get("draft"),
                    "mergeStateStatus": pr_details.get("mergeable_state"),
                    "updatedAt": pr_details.get("updated_at"),
