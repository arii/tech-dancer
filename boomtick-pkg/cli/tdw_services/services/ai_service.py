            ...
            try:
                ...
                if any(sf in file_path for sf in sensitive_files):
                    ...
                    for line in resolved.splitlines():
                        ...
                        if any(kw in line for kw in ["node", "pnpm", "uses:", "@v", "packageManager"]) or ".nvmrc" in file_path or ".node-version" in file_path:
                    ...
                    if changes:
                         ...
                         if any(f["severity"] == "error" for f in findings):
                              log_warn(f"AI resolved conflict but introduced version downgrade in {file_path}. Rejecting resolution.")
                              return False
            except Exception as e:
                log_warn(f"Error during version downgrade check: {e}")
