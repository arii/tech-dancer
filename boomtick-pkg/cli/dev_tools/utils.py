...
try:
    from dev_tools.tdw_services.utils import log_info, log_error, log_warn
except ImportError:
...
def log_ai_run(entry: dict):
    try:
        log_dir = os.path.join(os.getcwd(), "boomtick-pkg/cli/dev_tools", "logs", "ai")
...
def get_github_token() -> Optional[str]:
    ...
    token = os.getenv("GITHUB_TOKEN") or os.getenv("GH_TOKEN") or os.getenv("PAT_TOKEN")
    if token:
        return token
    try:
...
class GHAConfigManager:
    ...
    def __init__(self):
        ...
        try:
            self.config_path = str(files("dev_tools").joinpath("config.json"))
        except Exception:
