...
from dev_tools.tdw_services.services.ai_service import AIClient
from dev_tools.tdw_services.services.jules import JulesClient
from dev_tools.tdw_services.utils import log_error
from dev_tools.tdw_services.handlers.command_handler import CommandHandler
from utils import (
    get_github_token,
    get_github_client,
    get_repo_name,
...
class Orchestrator:
    ...
    def evaluate_pr_heuristics(self, pr: Dict[str, Any], diff: str, checks: Dict[str, Any]) -> str:
        ...
        if pr.get('mergeable') is False:
            ...
            feedback += f"  - *Fix:* Pull `{base_branch_name}` into your branch, resolve the conflicts (e.g., via `python3 boomtick-pkg/cli/dev_tools/td_cli.py gh conflicts`), and force push.\n"
    ...
    def audit_pr(self, pr_number: int, fetch: bool = False, audit: bool = False, submit: bool = False, cleanup: bool = False, dry_run: bool = True, event=None):
        review_dir = os.path.join(os.getcwd(), "boomtick-pkg/cli/dev_tools", "logs", "reviews")
    ...
    def repair_local(self, logs_path=None, stdin=False, worktree=False):
        ...
        original_cwd = os.getcwd(); repair_script = os.path.abspath(os.path.join(original_cwd, "boomtick-pkg/cli/dev_tools", "repair.py"))
...
- Notes or documented limitations"""
    ...
    def generate_ux_report(self):
        ...
        """
        from dev_tools.tdw_services.ux_report import generate_report
        generate_report()
