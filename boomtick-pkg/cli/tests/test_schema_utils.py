...
@mock_cli.command(name="test")
@click.option("--flag", help="A test flag")
@click.argument("arg")
def test_cmd(flag, arg):
    pass
...
def test_get_command_by_path():
    assert get_command_by_path(mock_cli, "test") == test_cmd
    assert get_command_by_path(mock_cli, "sub") == sub
    assert get_command_by_path(mock_cli, "sub nested") == nested_cmd
    assert get_command_by_path(mock_cli, "invalid") is None
    assert get_command_by_path(mock_cli, "") == mock_cli

def test_collect_commands_flat():
    schema = collect_commands(test_cmd, prefix="test")
...
def test_collect_commands_group():
    schema = collect_commands(mock_cli)
    assert "test" in schema
