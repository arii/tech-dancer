...
1. **Consult `.agent-context.json`** — ground truth for repository structure,
   ...
   `.githooks/`.
2. **Consult `cli/dev_tools/cli-schema.json`** — canonical authority for local CLI
...
python3 cli/dev_tools/td_cli.py gh pr-diff 2280
...
| **Tier 2** | `cli/dev_tools/td_cli.py` | Only when a Tier 1 tool is unavailable or the CLI provides logic not yet exposed via MCP |
