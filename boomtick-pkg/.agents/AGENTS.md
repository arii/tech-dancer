...
| **Tier 2** | `boomtick-pkg/cli/dev_tools/td` | Only when a Tier 1 tool is unavailable or the CLI provides logic not yet exposed via MCP |
...
| **GitHub** | Audit PR | `github.audit_pr` | `td gh audit-pr <PR> --fetch --audit` | - |
...
| **Agent** | Dispatch Jules | `jules.create_session` | `td agent dispatch <BRANCH> <TASK>` | - |
| **Agent** | Fix CI | `jules.fix_ci` | `td agent fix-ci --pr-number <PR>` | - |
| **Agent** | Sync Sessions | `jules.sync_sessions` | `td agent sync` | - |
| **Agent** | Send Message | `jules.send_message` | `td agent send <ID> <MSG>` | - |
| **Agent** | Get Messages | `jules.get_messages` | `td agent messages <ID>` | - |
