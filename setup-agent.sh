...
install_python_deps() {
  ...
  if [ -f "${CLI_ROOT}/pyproject.toml" ]; then
    ...
    local pkg_spec="${CLI_ROOT}/"
    [ "$INSTALL_AI_DEPS" = "1" ] && pkg_spec="${CLI_ROOT}/[ai]"
    ...
    if [ "$INSTALL_AI_DEPS" = "1" ] && [ -f "${CLI_ROOT}/requirements-ai.txt" ]; then
      pip_install --root-user-action=ignore -r "${CLI_ROOT}/requirements-ai.txt"
    fi
...
install_node_deps() {
  ...
  if [ -d "boomtick-pkg" ] && [ -f "boomtick-pkg/package.json" ]; then
    log "Installing Node dependencies for boomtick-pkg submodule..."
    (
      cd boomtick-pkg
      if [ -f "pnpm-lock.yaml" ]; then
        pnpm install --frozen-lockfile || pnpm install --no-frozen-lockfile
...
run_validation() {
  ...
  if have pnpm; then
    ...
    if have python3 && [ -f "$CLI_ROOT/dev_tools/schema_gen.py" ]; then
      ...
      (
        ...
        if [ -d "boomtick-pkg" ]; then
          (cd boomtick-pkg && pnpm --filter @arii/boomtick-mcp run sync-contracts)
        else
    ...
    if [ -d "boomtick-pkg" ]; then
      (cd boomtick-pkg && pnpm --filter @arii/boomtick-mcp run sync:mcp-schemas) || true
    else
