...
def handle_detect_conflicts(args):
    ...
    log_output(args, "✅ No potential merge conflicts detected." if not conflicts else "", {"status": "success", "conflicts": formatted})
...
def handle_status_board(args):
    ...
    log_output(args, "", {"status": "success", "work": prs_data})
...
def handle_ratchet_any(args):
    ...
    if current > baseline:
        ...
        log_output(args, f"❌ Error: {msg}", {"status": "error", "message": msg, "data": res})
        sys.exit(1)
...
def handle_migrate_tokens(args):
    ...
    log_output(args, "", {"status": "success", "matches": matches})
...
def handle_update_issues(args):
    ...
    log_output(args, "", {"status": "success", "updates": updates})
...
def handle_pre_submit(args):
    ...
    try:
        ...
        log_output(args, "✅ Pre-submission checks passed.", {"status": "success", "results": results})
    except Exception as e:
        log_output(args, f"❌ Pre-submission checks failed: {e}", {"status": "error", "message": str(e), "results": results})
        sys.exit(1)

def handle_resolve_conflicts(args):
    """Scan for conflict markers and use MergeLlama to resolve them."""
    conflict_files = []
    exclude_dirs = {'.git', 'node_modules', 'dev-tools', 'dist', 'build'}
    ...
    for root, dirs, files in os.walk('.'):
        ...
        for file in files:
            ...
            try:
                # We do a quick check for markers without loading huge files if possible,
                # but for simplicity and safety (binary files), we just check small text-like files.
                if not any(file.endswith(ext) for ext in {'.ts', '.tsx', '.js', '.jsx', '.json', '.md', '.css', '.html'}):
                    continue
    ...
    log_output(args, f"🔍 Found {len(conflict_files)} file(s) with conflicts. Starting MergeLlama...")
...
def handle_repair(args):
    ...
    try:
        ...
        if proc.returncode == 0:
            log_output(args, "✅ Repair process completed.", {
                "status": "success",
                "worktree": worktree_path,
                "branch": branch_name if args.worktree else None
            })
            if not args.json and worktree_path:
                print(f"👉 Inspect your fixed code at: {worktree_path}")
                print(f"   Branch: {branch_name}")
        else:
            log_output(args, f"❌ Repair process failed with code {proc.returncode}", {"status": "error", "code": proc.returncode})
...
def handle_audit_gate(args):
    ...
    if current_count > baseline_count:
        ...
        log_output(args, f"❌ Error: {msg}", {"status": "error", "message": msg, "data": {"current": current_count, "baseline": baseline_count}})
        sys.exit(1)

    log_output(args, "✅ No new violations introduced.", {"status": "success", "data": {"current": current_count, "baseline": baseline_count}})
...
def handle_fix_ci(args):
    ...
    log_output(args, "", {"status": "success", "session": session_name, "branch": branch})
...
def handle_manage_reviews(args):
    ...
    log_output(args, "", {"status": "success", "prs": prs_data})
...
def main():
    ...
    for cmd, func in [("validate-issue", handle_validate_issue), ("conflicts", handle_conflicts), ("detect-conflicts", handle_detect_conflicts),
        ...
        if cmd == "validate-issue":
            ...
            p.add_argument("--api-key", help="JULES_API_KEY (falls back to JULES_API_KEY env var)")
            add_execution_args(p)
        elif cmd == "repair":
    ...
    try:
        ...
        log_output(args, f"❌ Error: {e.message}", {"status": "error", "message": e.message, "code": e.code, "data": e.data})
        sys.exit(e.code)
    except Exception as e:
        log_output(args, f"❌ Unexpected Error: {str(e)}", {"status": "error", "message": str(e)})
        if not args.json:
            raise e
        sys.exit(1)
