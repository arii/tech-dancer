...
class Orchestrator:
    ...
    def audit_pr(self, pr_number: int, fetch: bool = False, audit: bool = False, submit: bool = False, cleanup: bool = False, dry_run: bool = True, event=None, mobile_url: Optional[str] = None, mobile_routes: Tuple[str, ...] = ()):
        if mobile_routes and not mobile_url:
            raise CLIError("--mobile-route requires --mobile-url.")
        if mobile_url and not audit:
            raise CLIError("--mobile-url requires --audit.")
        ...
        if audit:
            ...
            if mobile_url:
                mobile_audit = self.audit_mobile(mobile_url, list(mobile_routes))
                res["mobile_audit"] = mobile_audit
                for result in mobile_audit.get("results", []):
                    for finding in result.get("findings", []):
                        auto_findings.append({
                            "path": f"MOBILE UX {result['route']} @ {result['viewport']['width']}px",
                            "issue": f"{finding['type']}: {finding['message']} Selector: {finding['selector']}",
                            "severity": "major",
                        })
    ...
    def audit_mobile(self, url: str, routes: Optional[List[str]] = None, output_dir: str = "test-results/mobile-ux-audit") -> Dict[str, Any]:
        """Run Chromium at mobile widths and return actionable viewport-overflow findings."""
        command = ["pnpm", "exec", "tsx", "scripts/mobile-ux-audit.ts", "--url", url, "--output-dir", output_dir]
        for route in routes or ["/"]:
            command.extend(["--route", route])
        result = run_command(command, check=False, log_on_error=False)
        if not result.stdout.strip():
            details = result.stderr.strip() or "Mobile browser audit did not return a JSON report."
            raise CLIError(f"Mobile UX audit failed: {details}", code=result.returncode or 1)
        try:
            report = json.loads(result.stdout)
        except json.JSONDecodeError as error:
            raise CLIError(f"Mobile UX audit returned invalid JSON: {error}", code=result.returncode or 1)
    ...
    def comment_pr(self, pr_number: int, body: Optional[str] = None, body_file: Optional[str] = None, dry_run: bool = True) -> Dict[str, Any]:
        ...
        if not dry_run:
            ...
            response = self.github.create_issue_comment(pr_number, comment_body)
            result["posted"] = True
