...
def get_ollama_synthesis_model() -> str:
    ...
    try:
        ...
        try:
            config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), "dev-tools", "project_config.json")
            if not os.path.exists(config_path):
                config_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "project_config.json")
...
def call_ollama(prompt: str, model: str = None, url: Optional[str] = None, max_retries: int = 3, schema = None, system_prompt: Optional[str] = None, json_format: bool = False) -> Optional[str]:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                print(f"Ollama connection error (attempt {attempt}/{max_retries}). Retrying in {sleep_time:.2f}s... Error: {e}", file=sys.stderr)
                time.sleep(sleep_time)
            else:
                print(f"Failed to connect to Ollama after {max_retries} attempts: {e}", file=sys.stderr)
                return None
