...
def process_pr(pr):
    ...
    try:
        ...
        # 3. Squash onto main
        # We use reset --soft to origin/main to collapse all changes into index
        # This handles 'squash all commits' requirement
        ...
        if run_cmd(["git", "status", "--porcelain"]):
            ...
            # If no changes remain, we might still want to push if the branch was different
            # but usually it means the PR is redundant.
            return
