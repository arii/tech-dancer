...
describe('agent-prime.mjs', () => {
  ...
  describe('generateAgentContext', () => {
    it('returns context with valid git and package data', () => {
      ...
      vi.mocked(child_process.execSync).mockImplementation((cmd, opts) => {
        if (cmd === 'git rev-parse HEAD:boomtick-pkg') return 'mocked-submodule-sha\n';
        if (cmd === 'git rev-parse HEAD') return 'mocked-git-sha\n';
        return '';
      });
