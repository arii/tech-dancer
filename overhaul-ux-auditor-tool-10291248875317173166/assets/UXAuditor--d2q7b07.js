 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Vd(r,e=[]){let t;switch(r){case"Browser":t=hl(ve());break;case"Worker":t=`${hl(ve())}-${r}`;break;default:t=r}const n=e.length?e.join(","):"FirebaseCore-web";return`${t}/JsCore/${Tr}/${n}`}/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class NE{bt(e){return Ae(e,cc)}getSessionToken(e){return this.bt(e).get("sessionToken").next((t=>{const n=t?.value;return n?Ie.fromUint8Array(n):Ie.EMPTY_BYTE_STRING}))}setSessionToken(e,t){return this.bt(e).put({name:"sessionToken",value:t.toUint8Array()})}}/**
 * @license
 * Copyright 2021 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const De="WebChannelConnection",Hr=(r,e,t)=>{r.listen(e,(n=>{try{t(n)}catch(s){setTimeout((()=>{throw s}),0)}}))};class er extends _w{constructor(e){super(e),this.a_=[],this.forceLongPolling=e.forceLongPolling,this.autoDetectLongPolling=e.autoDetectLongPolling,this.useFetchStreams=e.useFetchStreams,this.longPollingOptions=e.longPollingOptions}static u_(){if(!er.c_){const e=Qd();Hr(e,Wd.STAT_EVENT,(t=>{t.stat===ya.PROXY?x(De,"STAT_EVENT: detected buffering proxy"):t.stat===ya.NOPROXY&&x(De,"STAT_EVENT: detected no buffering proxy")})),er.c_=!0}}zo(e,t,n,s,i){const o=Ba();return new Promise(((c,u)=>{const h=new Kd;h.setWithCredentials(!0),h.listenOnce(Hd.COMPLETE,(()=>{try{switch(h.getLastErrorCode()){case yi.NO_ERROR:const m=h.getResponseJson();x(De,`XHR for RPC '${e}' ${o} received:`,JSON.stringify(m)),c(m);break;case yi.TIMEOUT:x(De,`RPC '${e}' ${o} timed out`),u(new D(P.DEADLINE_EXCEEDED,"Request time out"));break;case yi.HTTP_ERROR:const v=h.getStatus();if(x(De,`RPC '${e}' ${o} failed with status:`,v,"response text:",h.getResponseText()),v>0){let S=h.getResponseJson();Array.isArray(S)&&(S=S[0]);const k=S?.error;if(k&&k.status&&k.message){const N=(function($){const U=$.toLowerCase().replace(/_/g,"-");return Object.values(P).indexOf(U)>=0?U:P.UNKNOWN})(k.status);u(new D(N,k.message))}else u(new D(P.UNKNOWN,"Server responded with status "+h.getStatus()))}else u(new D(P.UNAVAILABLE,"Connection failed."));break;default:L(9055,{l_:e,streamId:o,h_:h.getLastErrorCode(),P_:h.getLastError()})}}finally{x(De,`RPC '${e}' ${o} completed.`)}}));const f=JSON.stringify(s);x(De,`RPC '${e}' ${o} sending request:`,s),h.send(t,"POST",f,n,15)}))}T_(e,t,n){const s=Ba(),i=[this.Ko,"/","google.firestore.v1.Firestore","/",e,"/channel"],o=this.createWebChannelTransport(),c={httpSessionIdParam:"gsessionid",initMessageHeaders:{},messageUrlParams:{database:`projects/${this.databaseId.projectId}/databases/${this.databaseId.database}`},sendRawJson:!0,supportsCrossDomainXhr:!0,internalChannelParams:{forwardChannelRequestTimeoutMs:6e5},forceLongPolling:this.forceLongPolling,detectBufferingProxy:this.autoDetectLongPolling},u=this.longPollingOptions.timeoutSeconds;u!==void 0&&(c.longPollingTimeout=Math.round(1e3*u)),this.useFetchStreams&&(c.useFetchStreams=!0),this.Go(c.initMessageHeaders,t,n),c.encodeInitMessageHeaders=!0;const h=i.join("");x(De,`Creating RPC '${e}' stream ${s}: ${h}`,c);const f=o.createWebChannel(h,c);this.E_(f);let m=!1,v=!1;const S=new yw({Jo:k=>{v?x(De,`Not sending because RPC '${e}' stream ${s} is closed:`,k):(m||(x(De,`Opening RPC '${e}' stream ${s} transport.`),f.open(),m=!0),x(De,`RPC '${e}' stream ${s} sending:`,k),f.send(k))},Ho:()=>f.close()});return Hr(f,Qr.EventType.OPEN,(()=>{v||(x(De,`RPC '${e}' stream ${s} transport opened.`),S.i_())})),Hr(f,Qr.EventType.CLOSE,(()=>{v||(v=!0,x(De,`RPC '${e}' stream ${s} transport closed`),S.o_(),this.I_(f))})),Hr(f,Qr.EventType.ERROR,(k=>{v||(v=!0,Rn(De,`RPC '${e}' stream ${s} transport errored. Name:`,k.name,"Message:",k.message),S.o_(new D(P.UNAVAILABLE,"The operation could not be completed")))})),Hr(f,Qr.EventType.MESSAGE,(k=>{if(!v){const N=k.data[0];F(!!N,16349);const C=N,$=C?.error||C[0]?.error;if($){x(De,`RPC '${e}' stream ${s} received error:`,$);const U=$.status;let j=(function(H){const T=_e[H];if(T!==void 0)return Wf(T)})(U),te=$.message;U==="NOT_FOUND"&&te.includes("database")&&te.includes("does not exist")&&te.includes(this.databaseId.database)&&Rn(`Database '${this.databaseId.database}' not found. Please check your project configuration.`),j===void 0&&(j=P.INTERNAL,te="Unknown error status: "+U+" with message "+$.message),v=!0,S.o_(new D(j,te)),f.close()}else x(De,`RPC '${e}' stream ${s} received:`,N),S.__(N)}})),er.u_(),setTimeout((()=>{S.s_()}),0),S}terminate(){this.a_.forEach((e=>e.close())),this.a_=[]}E_(e){this.a_.push(e)}I_(e){this.a_=this.a_.filter((t=>t===e))}Go(e,t,n){super.Go(e,t,n),this.databaseInfo.apiKey&&(e["x-goog-api-key"]=this.databaseInfo.apiKey)}createWebChannelTransport(){return Jd()}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class ut{constructor(e,t){if(!isFinite(e)||e<-90||e>90)throw new D(P.INVALID_ARGUMENT,"Latitude must be a number between -90 and 90, but was: "+e);if(!isFinite(t)||t<-180||t>180)throw new D(P.INVALID_ARGUMENT,"Longitude must be a number between -180 and 180, but was: "+t);this._lat=e,this._long=t}get latitude(){return this._lat}get longitude(){return this._long}isEqual(e){return this._lat===e._lat&&this._long===e._long}_compareTo(e){return G(this._lat,e._lat)||G(this._long,e._long)}toJSON(){return{latitude:this._lat,longitude:this._long,type:ut._jsonSchemaVersion}}static fromJSON(e){if(Os(e,ut._jsonSchema))return new ut(e.latitude,e.longitude)}}ut._jsonSchemaVersion="firestore/geoPoint/1.0",ut._jsonSchema={type:ye("string",ut._jsonSchemaVersion),latitude:ye("number"),longitude:ye("number")};/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
