        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                if k == "type" and v == "integer":
                    ...
                    # Only skip field/property titles, keep the top-level model title
                    # Actually, pydantic puts titles on every field.
                    # If we skip ALL titles, we lose the top level one too.
                    continue
                else:
...
print(f"Schema without title: {TestModel.model_json_schema(schema_generator=MCPGenerateJsonSchema)}")
