import{A as Je}from"./index-UeI99Ffc.js";const mn=`---
type: post
draft: true
status: draft
...
python boomtick-pkg/cli/aggregate_pr_context.py \\
  --target-branch main \\
  --output .devai/review-context.md
\`\`\`

> **Implemented:** \`boomtick-pkg/cli/td_cli.py gh audit-pr <PR_NUMBER> --fetch\` fetches PR diffs, CI logs, and linked issue context into a structured review packet. \`boomtick-pkg/cli/aggregate-prs.sh\` handles batch aggregation.
...
\`\`\`python
# boomtick-pkg/cli/submit_review.py
import json
...
> **Implemented:** \`boomtick-pkg/cli/submit_review.py\` handles \`APPROVE\`, \`REQUEST_CHANGES\`, and \`COMMENT\` states.
...
> **Experimental:** \`boomtick-pkg/cli/td_cli.py ai repair\` can be triggered when CI fails. A GitHub Actions workflow (\`jules-fix-trigger.yml\`) exists to initiate repair sessions. Treat the output as a suggestion; always review before merge.
...
python boomtick-pkg/cli/aggregate_pr_context.py > .devai/review-context.md
python boomtick-pkg/cli/ai_review.py .devai/review-context.md > .devai/review-result.json
python boomtick-pkg/cli/submit_review.py .devai/review-result.json
\`\`\`
