    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground md:flex-row">
      ...
      <main className="flex-1 min-h-screen md:ml-56 px-4 sm:px-6 md:px-10 py-6 md:py-14">
        ...
        <section id="contact-content" className="max-w-5xl">
          <p className="mb-4 text-xs font-bold tracking-[0.35em] uppercase text-muted-foreground">Contact</p>
          <h1 className="mb-4 text-3xl font-black sm:text-4xl md:text-5xl">Get in Touch</h1>
          <p className="mb-8 max-w-3xl text-sm text-foreground/72 sm:text-base">Have a question about West Coast Swing, consulting, project work, travel, gear, or the site itself? I’d love to hear from you.</p>
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm sm:p-6 md:p-8">
            <div className="mb-8 flex flex-col gap-2">
              ...
              <p className="max-w-2xl text-sm text-foreground/72">I’m open to new ideas, questions about reviews, or a good dance-scene conversation.</p>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <input id="contact-name" className="min-h-11 rounded-lg border border-border bg-background px-4 py-3 text-foreground outline-none transition-colors placeholder:text-foreground/45 focus:border-primary" placeholder="Your Name" />
              <label className="sr-only" htmlFor="contact-email">Your Email</label>
              <input id="contact-email" className="min-h-11 rounded-lg border border-border bg-background px-4 py-3 text-foreground outline-none transition-colors placeholder:text-foreground/45 focus:border-primary" placeholder="Your Email" />
            </div>
            <div className="mt-4">
              <label className="sr-only" htmlFor="contact-subject">Subject</label>
              <input id="contact-subject" className="min-h-11 w-full rounded-lg border border-border bg-background px-4 py-3 text-foreground outline-none transition-colors placeholder:text-foreground/45 focus:border-primary" placeholder="Subject" />
            </div>
            <div className="mt-4">
              <label className="sr-only" htmlFor="contact-message">Message</label>
              <textarea id="contact-message" className="min-h-44 w-full rounded-lg border border-border bg-background px-4 py-3 text-foreground outline-none transition-colors placeholder:text-foreground/45 focus:border-primary" placeholder="Message" />
            </div>
