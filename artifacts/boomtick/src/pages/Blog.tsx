    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground md:flex-row">
      ...
      <main className="flex-1 min-h-screen md:ml-56 px-4 sm:px-6 md:px-10 py-6 md:py-14">
        <section className="max-w-6xl">
          <p className="mb-4 text-xs font-bold tracking-[0.35em] uppercase text-muted-foreground">Insights</p>
          <h1 className="mb-4 text-3xl font-black sm:text-4xl md:text-5xl">Blog Posts</h1>
          <p className="mb-8 max-w-3xl text-sm text-foreground/72 sm:text-base">A searchable collection of West Coast Swing posts covering travel, lifestyle, gear reviews, and dance research.</p>
          <div className="mb-8 flex flex-wrap gap-2 rounded-xl border border-border bg-card/70 p-3 shadow-sm">
            {blogFilters.map((item) => (
              <button
                ...
                className={`rounded-full border px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 ${activeFilter === item ? "border-secondary bg-secondary text-background shadow-sm" : "border-border bg-background/40 text-muted-foreground hover:border-primary/40 hover:bg-background/70 hover:text-foreground"}`}
              >
                {item}
              </button>
            ))}
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {visiblePosts.map((post) => (
              <article key={post.href} className="flex min-h-[260px] flex-col gap-4 rounded-xl border border-border bg-card/90 p-5 shadow-sm transition-colors hover:border-primary/30">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <time className="text-[11px] font-mono text-foreground/75">{post.date}</time>
                </div>
