        ...
        <div className="border-b border-border bg-background/98 backdrop-blur">
          <div className="flex h-11 items-center justify-between px-2.5">
            <Link href="/" className="inline-flex items-center" data-testid="link-home-logo-mobile">
              <div className="origin-left scale-[0.34] -translate-x-7 -translate-y-1">
                <Logo />
              </div>
            </Link>
            <button type="button" onClick={() => setOpen((value) => !value)} className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border bg-card text-foreground" data-testid="button-toggle-nav" aria-label="Toggle navigation">
              {open ? <X size={16} /> : <Menu size={16} />}
            </button>
          </div>
        </div>
        {open ? (
          <nav className="absolute left-0 right-0 border-b border-border bg-card px-2 py-1.5 shadow-2xl">
            {primaryNavigation.map((item) => (
