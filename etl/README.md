...
The tool automatically generates Markdown files in `content/studies/`.
- These files are utilized by the React frontend via `import.meta.glob`.
- To maintain repository hygiene, these autogenerated files are ignored by Git (configured in `.gitignore`).
