# Style Migration Plan to Main Branch - Incremental Rollout Guide

This document provides a comprehensive, step-by-step guide for migrating the UI/UX and style updates from the current development branch into the `main` branch. The plan is structured into sequential phases, allowing for atomic merges into separate feature branches. Each instruction aims to be self-contained, enabling replication of changes directly from this document.

## 1. Context and Architectural Assumptions (Origin/Main Branch)

This migration plan is formulated with the following understanding of the `origin/main` branch's architecture and conventions:

-   **Jamstack Application**: The project is a Jamstack application, implying client-side rendering with server-side generation/pre-rendering for performance and SEO.
-   **Next.js with Custom Server**: It's a Next.js application, potentially using `react-router-dom` for client-side routing, alongside a custom Node.js/Express server (`server.ts`).
-   **TypeScript**: The codebase is primarily TypeScript.
-   **Styling**: It leverages Tailwind CSS (as indicated by `tailwind.config.mjs` and class usage) alongside Material-UI (MUI) components, or a hybrid approach where Tailwind provides utility-first styling for custom components.
-   **Package Management**: `pnpm` is the preferred package manager.
-   **File Structure**: A modular structure is assumed, with `src/components/`, `src/layouts/`, `src/features/`, `src/pages/`, `src/config/`, and `content/` directories.

## 2. Summary of Changes

This migration incorporates significant improvements focusing on:
-   **Responsive Layouts**: Ensuring correct rendering across desktop, tablet, and mobile, preventing content overlap, and enhancing the "thin mode" (very narrow viewport) experience.
-   **Design System Adherence**: Refactoring components to leverage primitive `Box`, `Stack`, and `Grid` components, and applying consistent spacing, typography, and color tokens. This aligns with the existing Tailwind CSS setup.
-   **Accessibility & Semantics**: Fixing HTML nesting issues (e.g., `<a>` within `<a>`), improving form element attributes, and ensuring proper content rendering.
-   **Content Integration**: Transitioning hardcoded content to dynamic fetching where appropriate (e.g., events), and updating existing content. This enhances the Jamstack content management flow.
-   **Bug Fixes**: Addressing various console errors and visual regressions, improving overall application stability.

## 3. Migration Phases

The migration will be divided into three main phases, designed for sequential integration. Each phase is intended to be a single, independently mergeable unit of work.

---

### Phase 1: Content & Data Structure Updates
...
**Estimated PRs**: 1-2 feature branches, merged sequentially.

#### Detailed Changes:

1.  **Setup Dynamic Event Data**
