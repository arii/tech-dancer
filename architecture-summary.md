...
## Tool Registration
The `boomtick-mcp/src/mcp/server.ts` was updated to explicitly register purely task-oriented tools for Jules (`jules.create_session`, `jules.get_session`, `jules.list_sessions`, `jules.cancel_session`, `jules.get_pr`, `jules.send_message`). Local agent endpoints have been completely removed.
