import{A as Ye}from"./index-WsC0MqkL.js";const gn=`---
type: post
draft: true
status: draft
...
$&`).replace(/(?:^|\n)([\t ].*)(?:([\n\t ]*)\n(?![\n\t ]))?/g,"$1$2").replace(/\n+/g,`$&${c}`);let _=!1;const A=Me(s,!0);a!=="folded"&&e!==T.BLOCK_FOLDED&&(A.onOverflow=()=>{_=!0});const y=Be(`${w}${S}${m}`,c,Xe,A);if(!_)return`>${v}
...
`:"",w){const A=c(w);_+=`
${x(A,t.indent)}`}S===""&&!t.inFlow?_===`
`&&k&&(_=`

`):_+=`
${t.indent}`}else if(!m&&O(e)){const A=S[0],y=S.indexOf(`
`),E=y!==-1,U=t.inFlow??e.flow??e.items.length===0;if(E||!U){let z=!1;if(E&&(A==="&"||A==="!")){let L=S.indexOf(" ");A==="&"&&L!==-1&&L<y&&S[L+1]==="!"&&(L=S.indexOf(" ",L+1)),(L===-1||y<L)&&(z=!0)}z||(_=`
${t.indent}`)}}else(S===""||S[0]===`
...
`:case"\r":case"	":return!0;default:return!1}}const Ot=new Set("0123456789ABCDEFabcdef"),ai=new Set("0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-#;/?:@&=+$_.!~*'()"),_e=new Set(",[]{}"),ri=new Set(` ,[]{}
