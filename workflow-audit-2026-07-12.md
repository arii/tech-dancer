...
*   **`.github/workflows/deploy.yml`**:
    *   Pinned `actions/download-artifact` to `v3`.

*Note: Most workflow files such as `auto-conflict-resolver.yml`, `prune-stale-previews.yml`, `reusable-gate.yml`, `update-snapshots.yml`, `wcs_etl.yml`, and `deploy-image.yml` were already correctly pinned to the latest major action versions and did not require any modifications.*
