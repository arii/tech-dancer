...
## Overview
This documentation outlines the architecture of "Jules" (an external task-oriented execution system) inside the `boomtick-pkg/mcp` service.

## Types
1. Created `boomtick-pkg/mcp/src/tools/types.ts`.
...
**Jules Tools** (`boomtick-pkg/mcp/src/tools/jules/`):
...
## Tool Registration
The `boomtick-pkg/mcp/src/mcp/server.ts` was updated to register tools for Jules (`jules.create_session`, `jules.get_session`, `jules.list_sessions`, `jules.cancel_session`, `jules.get_pr`).
