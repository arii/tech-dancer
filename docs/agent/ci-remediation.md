...
1.  **Extract failure details**: Use the `extract_failing_info` helper (via `td gh audit-pr <PR> --fetch`) or manually search for error patterns:
...
1.  **Fetch PR context**: `td gh audit-pr <PR_NUMBER> --fetch`.
