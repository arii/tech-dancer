...
export interface BudgetInputContextOptions {
...
export function budgetInputContext(
  ...
  options: BudgetInputContextOptions
): { diffText: string; externalText: string } {
  const maxInputChars = options.maxInputChars ?? 24000;
  const remainingBudgetForDiffAndContext = maxInputChars - options.systemPromptLength;

  let diffText = `DIFF:\n\n${diffContent}`;
  let externalText = externalContext
