...
experiments/deterministic_loop.py: Explores a rigid while loop that acts on basic string matching from the CLI tools (e.g., triggering a task when it sees "conflicts" in td_cli.py gh conflicts).
...
def wait_for_agent(session_id):
    # Polling logic from the main architecture
    pass
