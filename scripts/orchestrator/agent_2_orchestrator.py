...
from dev_tools.utils import run_command as run_cli
import re
import time

def get_session_id():
    # Helper replacing the local definition
    stdout = run_cli(["td-cli", "agent", "sync"])
...
def wait_for_agent(session_id, poll_interval=10, timeout=300, max_retries=30):
    ...
    while True:
        ...
        messages = run_cli(["td-cli", "agent", "messages", session_id])
        if messages:
