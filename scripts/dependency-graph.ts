...
export function generateGraph(): DependencyGraph {
  ...
  try {
    execSync(`npx depcruise src --config .dependency-cruiser.config.mjs --output-type json > ${tmpFile}`, { stdio: 'pipe' });
  } catch {
    // dependency-cruiser might return non-zero if there are validation errors, but we still get output
    console.warn('dependency-cruiser exited with an error, but output might still be available.');
  }
...
export function findReverseDependencies(graph: DependencyGraph, changedFiles: string[]): string[] {
  ...
  for (const module of graph.modules) {
    ...
    for (const dep of module.dependencies) {
      // Clean resolved path
      const normalizedResolved = path.normalize(dep.resolved).replace(/\\/g, '/');
