...
async function reviewRoute(
  ...
  if (!fs.existsSync(beforePath) || !fs.existsSync(afterPath)) {
    console.warn(`⚠️  Skipping visual review for ${summary.route}: Missing BEFORE or AFTER screenshots.`);
    return {
      route: summary.route,
      severity: summary.severity,
      differencePercent: summary.differencePercent,
      feedback: 'Visual review skipped due to missing BEFORE/AFTER screenshots.',
      tokens: 0,
      cost: 0,
    };
  }
...
function generateMarkdownReport(reviews: RouteReview[], totalRoutes: number): string {
...
`).join('\n---\n');
  ...
  let truncationNote = '';
  if (totalRoutes > reviews.length) {
    truncationNote = `\nNote: ${totalRoutes - reviews.length} additional routes were detected with visual changes but were not reviewed due to the configured limit.\n`;
  }
...
async function main(): Promise<void> {
  ...
  // Limit to top routes by difference percentage to manage costs
  const maxRoutesEnv = process.env.AGENT_REVIEW_MAX_ROUTES;
  const MAX_ROUTES = maxRoutesEnv ? parseInt(maxRoutesEnv, 10) : 5;
  ...
  const report = generateMarkdownReport(reviews, totalRoutes);
