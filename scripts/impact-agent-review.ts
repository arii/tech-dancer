...
function createModel(): ChatGoogleGenerativeAI {
  ...
  if (!apiKey) {
    console.warn('⚠️ Missing GEMINI_API_KEY environment variable. Skipping agent review.');
    process.exit(0);
  }
...
async function hasAlreadyReviewed(token: string, repo: string, prNumber: string, commitSha: string): Promise<boolean> {
  ...
  try {
    ...
    const comments = await response.json() as Array<{ user: { type: string }, body: string }>;
    const existing = comments.find(c =>
      c.user.type === 'Bot' &&
      c.body.includes('## 👁️ Visual Review Agent') &&
...
async function main(): Promise<void> {
  ...
  if (token && repo && prNumber) {
    const alreadyReviewed = await hasAlreadyReviewed(token, repo, prNumber, commitSha);
    if (alreadyReviewed) {
      console.log(`✅ Visual Review Agent already reviewed commit ${commitSha}. Skipping.`);
      return;
    }
  }
...
main().catch(error => {
  ...
  // Do not fail CI if the agent errors out
  process.exit(0);
});
