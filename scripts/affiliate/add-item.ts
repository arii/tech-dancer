...
async function main() {
  ...
  if (createPr && !dryRun) {
    handlePullRequest(id, title, imagePath, targetContent);
  }
}

function handlePullRequest(slug: string, title: string, imagePath?: string, targetContentPath?: string) {
...
Generated by BoomTick Affiliate Tooling.
    ...
    if (prRes.status !== 0) {
      console.warn('PR creation failed (it might already exist).');
    } else {
