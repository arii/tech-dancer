...
/**
 * Writes analysis artifacts to the filesystem.
 */
function writeArtifacts(report: {
  ...
  const { changedFiles, routes, impactLevel } = report;
  ...
  const severityEmoji = impactLevel === 'HIGH' ? '🔴' : impactLevel === 'MEDIUM' ? '🟡' : '🟢';
  const markdown = `## ${severityEmoji} Deployment Impact Analysis
> **Impact Level:** ${impactLevel}
### 👁️ Visual Review Required
${routes.length > 0 ? routes.map(url => `- [${url}](https://boomtick.blog${url})`).join('\n') : '_None detected (code-only change)_'}
<details><summary><b>📝 Changed Files (${changedFiles.length})</b></summary>
...
*Generated by Boomtick Impact Analyzer*`;
  ...
  if (routes.length > 0) routes.forEach(url => console.log(`  - ${url}`));
...
async function main() {
  ...
  try {
    ...
    if (changedFiles.length === 0) {
      ...
      writeArtifacts({ changedFiles: [], affectedPages: [], routes: [], visualReviewRequired: [], impactLevel: 'LOW' });
      return;
    }
    ...
    writeArtifacts({ changedFiles, affectedPages, routes: allUrls, visualReviewRequired: allUrls, impactLevel: severity });
  } catch (error: unknown) {
