...
import { summarizeDomCompact, type CompactRouteMetric } from './impact/metric-calculator';
...
function main(): void {
  ...
  const summaries: CompactRouteMetric[] = [];
  ...
  for (const route of routes) {
    ...
    const totalChanged = metrics.nodes[0] + metrics.nodes[1];

    const summary: CompactRouteMetric = {
      route,
      severity: domSeverity(totalChanged),
      metrics
    };
