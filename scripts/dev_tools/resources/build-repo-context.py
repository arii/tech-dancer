...
def build_repo_context():
    ...
    # scripts/build-repo-context.py -> scripts
    package_root = script_path.parent.parent

    # Check if we are in a monorepo or a standalone repo
    if (package_root.parent / "package.json").exists() and (package_root.parent / "scripts").exists():
        # Monorepo mode
    ...
    # If we are running from a location that isn't scripts/scripts,
    # fallback to discovery based on workspace.json
    if package_root.name != "scripts" or not (package_root / "workspace.json").exists():
        ...
        if found_pkg:
            ...
            package_root = repo_root if (repo_root / "workspace.json").exists() else repo_root / "scripts"
