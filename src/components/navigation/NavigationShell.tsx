...
const NavigationShell = () => {
  ...
  return (
    <>
      <aside className="fixed top-0 left-0 hidden h-full w-56 flex-col border-r border-border bg-card md:flex z-40" aria-label="Main Navigation">
        <div className="border-b border-border px-4 py-4">
          <Logo />
        </div>
        <nav className="flex-1 overflow-y-auto py-4" aria-label="Primary">
          <button type="button" aria-label="Search" onClick={handleSearchClick} className="w-full group flex min-h-11 items-center gap-3 px-6 py-3 text-sm text-foreground/80 transition-colors hover:bg-muted/50 hover:text-foreground focus-visible:bg-muted/50 focus-visible:text-foreground">
          ...
          {primaryNavigation.map((item) => (
            <Link key={item.label} to={item.href} data-testid={`nav-${item.label.toLowerCase().replace(/\s+/g, "-")}`} className="group flex min-h-11 items-center gap-3 px-6 py-3 text-sm text-foreground/80 transition-colors hover:bg-muted/50 hover:text-foreground focus-visible:bg-muted/50 focus-visible:text-foreground">
      ...
      <div className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur md:hidden">
        ...
        {open ? (
          <nav className="border-t border-border bg-card px-3 py-3" aria-label="Mobile Navigation">
            <button type="button" aria-label="Search" onClick={handleSearchClick} className="w-full flex min-h-11 items-center gap-3 rounded-lg px-4 py-3 text-sm text-foreground/80 transition-colors hover:bg-muted/50 hover:text-foreground focus-visible:bg-muted/50 focus-visible:text-foreground">
