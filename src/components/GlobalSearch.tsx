...
export function GlobalSearch() {
  ...
  const highlight = useCallback((text: string) => {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === query.toLowerCase()
        ? <Box as="span" key={i} radius="sm" paddingX={0.5} className="bg-accent/20 text-accent font-bold">{part}</Box>
        : part
    );
  }, [query]);
  ...
  return createPortal(
    <>
      {/* Backdrop - High z-index, portaled to body, simple div to avoid role conflicts */}
      <div
        data-testid="search-backdrop"
        onClick={close}
        className="fixed inset-0 bg-black/40 backdrop-blur-sm z-search cursor-pointer"
      />

      {/* Modal Container */}
      <div className="fixed inset-0 z-search flex justify-center items-start pt-16 lg:pt-20 pointer-events-none">
        <Box
          width="full"
          maxWidth="3xl"
          maxHeight="85vh"
          ...
          border
          shadow="topOverlay"
          className="border-accent/20 pointer-events-auto"
