...
export function ContentCard({ slug, title, category, excerpt, date, image, basePath, content, ...motionProps }: ContentCardProps) {
  ...
  return (
    <Stack
      ...
      shadow="standard"
      overflow="hidden"
      className="group hover:border-accent hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
      {...motionProps}
    >
        ...
        <Stack gap={3}>
          <Box display="flex" align="center" gap={3}>
            ...
            <Text variant="mono" size="xs" color="dim" uppercase tracking="widest" className="flex items-center gap-1">
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={image ? "opacity-50" : "opacity-30"}><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
              {rt} min
            </Text>
          </Box>
