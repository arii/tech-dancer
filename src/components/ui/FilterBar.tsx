...
export function FilterBar({ categories }: FilterBarProps) {
  ...
  return (
    <Box border="b" className="relative w-full bg-surface/80 backdrop-blur-md sticky top-0 z-40" paddingY={5}>
      <Box className="overflow-x-auto no-scrollbar">
        <Stack direction="row" gap={4} className="min-w-max px-6">
          {categories.map((cat) => (
            <Box
              key={cat}
              as="button"
      ...
      <Box
        ...
        inset="right"
        width={12}
        className="bg-gradient-to-l from-surface to-transparent pointer-events-none lg:hidden"
      />
    </Box>
  );
}
