...
export function CardImagePlaceholder({ image, category, title }: CardImagePlaceholderProps) {
  ...
  else if (norm.includes('gear')) surfaceVariant = 'accent';
  ...
  return (
    ...
    >
      ...
      <Box className="absolute top-4 left-4">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[#090914]/92 backdrop-blur-md border border-amber-400/40 rounded-full shadow-[0_0_16px_rgba(245,158,11,.14)]">
          {(() => {
            const icon = getCategoryIcon(category);
            return React.createElement(icon, { className: "w-3.5 h-3.5 text-amber-400", strokeWidth: 2.5 });
          })()}
