...
export function EventSidebar({ event, ...props }: EventSidebarProps) {
  const startDate = event?.startDate || props.startDate;
  const earlyBirdDate = event?.earlyBirdDate || props.earlyBirdDate;
  const hotelCutoffDate = event?.hotelCutoffDate || props.hotelCutoffDate;

  const reminders = startDate ? [
    {
      label: 'Flight Tracking',
      date: new Date(new Date(startDate).getTime() - 90 * 24 * 60 * 60 * 1000),
      icon: Plane,
    ...
    {
      label: 'Comp Signups',
      date: new Date(new Date(startDate).getTime() - 14 * 24 * 60 * 60 * 1000),
      icon: Users,
      description: 'Registration for competitions typically closes 14 days prior.'
    },
    {
      label: 'Cancel Safety',
      date: new Date(new Date(startDate).getTime() - 5 * 24 * 60 * 60 * 1000),
      icon: ShieldAlert,
  ...
  return (
    <Box as="aside">
      <Stack gap={8} className="sticky top-24">
        {/* Quick Intelligence - Only show if we have an event */}
        {event && (
