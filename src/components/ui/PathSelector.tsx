        ...
        return (
          ...
          >
            ...
            <div className="relative z-20 p-6 md:p-8 h-full flex flex-col justify-end bg-gradient-to-t from-black/80 via-black/20 to-transparent">
              <h2
                className={`${path.titleClass} font-display font-black text-white transition-transform duration-500 group-hover:translate-x-2 mb-3`}
              >
                {path.title}
              </h2>
              <ul className="space-y-2 mb-2 font-mono text-xs md:text-sm tracking-widest uppercase text-white font-bold opacity-80 group-hover:opacity-100 transition-opacity">
                {path.links.map((link) => (
                  <li key={link.text}>
                    <NavLink
