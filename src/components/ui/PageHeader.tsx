...
export function PageHeader({ label, title, description, as = "h1" }: PageHeaderProps) {
  return (
    <Box paddingBottom={10} className="border-b border-slate-200">
      <Stack gap={4}>
        <Text variant="mono" size="xs" color="dim" weight="font-semibold" uppercase={true} >
          {label}
        </Text>
...
export function SectionHeader({ label, title, children }: { label: string; title: string; children?: React.ReactNode }) {
  return (
    <Box display="flex" justify="between" align="end" border="b" paddingBottom={4} className="border-slate-200">
      <Stack gap={1}>
        <Text variant="mono" size="xs" color="dim" weight="font-semibold" >{label}</Text>
        <Text variant="displayLower" size="3xl" weight="font-black" className="text-accent-navy">{title}</Text>
      </Stack>
      {children}
    </Box>
  );
