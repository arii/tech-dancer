...
export default function Home() {
  return (
    <Box as="main" className="pb-safe-bottom">
      <SEO
        title="Home"
        description="BoomTick: Training tips, travel guides, and gear reviews for West Coast Swing dancers, Practical guides for West Coast Swing dancers — training notes, event travel tips, gear reviews, and tools for better dance weekends."
        schema={STATIC_SCHEMAS.HOME}
      />

      <Box display="grid" className="min-h-[420px] lg:grid-cols-[1fr_380px]">
        <HeroSection showWordmark={false} />
        <FeaturedGuidePanel />
      </Box>

      <Box className="lg:hidden" padding={4}
      >
        <Box as={NavLink} to="/gear/2023-11-01-travel-steamer" display="flex" direction="col" gap={3} padding={4} border radius="lg" className="group bg-surface">
          <Box width="full" height={48} radius="md" overflow="hidden" className="bg-surface-alt">
            <img src={`${ASSET_PREFIX}/assets/gear/travel-steamer.jpg`} alt="WCS Travel Pack" className="h-full w-full object-cover" />
          </Box>
          <Stack gap={1}>
