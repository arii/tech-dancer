...
interface AuditInputProps extends CardVariants {
  label: string;
  value: string;
...
const AuditInput = ({ label, value, onChange, type = "text", placeholder, helpText, isPassword, onClear, id, name, ...variants }: AuditInputProps) => (
  <Stack gap={1}>
    <Stack
      direction="row"
      align="center"
      gap={3}
      padding={2}
      className={cardVariants(variants)}
    >
...
function ViewportAnalysisCard({ vp, data, activeReportUrl, ...variants }: ViewportAnalysisCardProps) {
  return (
    <Box className={cardVariants({ overflow: "hidden", ...variants })} minWidth={0}>
