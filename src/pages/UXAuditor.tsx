            ...
            {reports.map((report) => (
              ...
              >
                <Box
                  padding={2}
                  radius="full"
                  className={report.status === 'completed' ? 'bg-success/10 text-accent' : 'bg-warning/10 text-warning animate-pulse'}
                >
                        ...
                        <Box padding={8} surface="muted" display="flex" align="center" justify="center" border={{ base: 'b', md: 'r' }} minHeight={400} width={{ base: 'full', md: '41.666%' }}>
                          {imgUrl ? (
                            ...
                            <Box className="text-center" color="dim">
                              <ImageIcon marginX="auto" marginBottom={2} className="w-12 h-12 opacity-20" />
                                  ...
                                  <Box key={idx} padding={4} radius="xl" border={true} surface="default" shadow="sm" className="hover:border-accent transition-all">
                                    <Box display="flex" justify="between" align="start" marginBottom={2}>
                                      <Text variant="sans" size="sm" weight="font-black" display="flex" align="center" gap={2}>
                                        <div className={`h-2 w-2 rounded-full ${imp.severity > 7 ? 'bg-danger shadow-sm' : 'bg-warning'}`} />
                                        {imp.element}
                                      </Text>
