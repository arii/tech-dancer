...
import { PenTool, Send, Copy, Sparkles, Github, ArrowRight, ArrowLeft, Info, Badge as BadgeIcon } from 'lucide-react';
...
*Metadata: ${formData.seoTerms}*`;
  ...
  const applyAiJson = () => {
    try {
      ...
      setAiJson('');
      alert('AI JSON applied successfully!');
    } catch (e) {
      alert('Invalid JSON format. Please ensure you pasted a valid JSON object.');
    }
  };
          ...
          <div className="bg-bg border border-line p-8 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <textarea 
                ...
                className="w-full bg-bg border border-line px-4 py-3 text-sm font-sans focus:outline-none focus:border-accent transition-colors resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-mono font-bold uppercase tracking-widest text-text-dim">Tags (comma separated)</label>
                <input 
                  type="text" 
                  value={formData.tags}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {copied && <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-accent text-white text-[10px] py-1 px-2 font-mono whitespace-nowrap">COPIED_TO_CLIPBOARD</span>}
              </button>
            </div>
          </div>
...
Return only a refined version of the content body in Markdown format or as a JSON object with keys: title, tagline, description, commentary, tags.`;
            ...
            <div className="pt-4 border-t border-accent/10 space-y-3">
              ...
              <textarea 
                ...
                className="w-full bg-bg border border-line px-4 py-3 text-[10px] font-mono focus:outline-none focus:border-accent transition-colors"
              />
              <button 
                onClick={applyAiJson}
