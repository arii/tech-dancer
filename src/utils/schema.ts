  ...
  return {
    ...
    "itemListElement": products.map((product, index) => ({
      ...
      "item": {
        ...
        "image": product.imageUrl.startsWith('http') ? product.imageUrl : (product.imageUrl.startsWith(ASSET_PREFIX) ? `${BASE_URL}${product.imageUrl}` : `${BASE_URL}${ASSET_PREFIX}${product.imageUrl}`),
        "offers": {
          "@type": "Offer",
          "price": product.price,
          "priceCurrency": "USD",
