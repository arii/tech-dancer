...
function getToolIcon(tool: ResearchTool): LucideIcon {
  ...
  if (tool.id.includes("scraper") || tool.id.includes("pipeline"))
    return Activity;
...
function ToolImage({ tool, baseUrl }: { tool: ResearchTool; baseUrl: string }) {
  ...
  const src = tool.image.startsWith("/")
    ? `${baseUrl}${tool.image}`
    : tool.image;
  const alt =
    tool.imageAlt || `Screenshot of the ${tool.title} interface preview`;
...
function EngineeringToolCard({ tool }: { tool: ResearchTool }) {
  ...
  return (
    <Stack
      as="div"
      className={cardVariants({ interactive: false })}
      padding={0}
      gap={0}
      height="full"
    >
      ...
      <Stack
        ...
        align="start"
        textAlign="left"
        className="w-full hover:bg-surface-alt transition-colors"
      >
      ...
      {/* Mobile view */}
      <Stack display={{ base: "flex", md: "none" }} width="full">
        <Box
          as="button"
          display="flex"
          align="center"
        ...
        >
          <Box display="flex" align="center" gap={3}>
            <Icon icon={getToolIcon(tool)} size="sm" color="dim" />
            <Text size="sm" weight="font-bold" color="body">
              {tool.title}
            </Text>
          </Box>
          <Box display="flex" align="center" gap={3}>
        ...
        {isExpanded && (
          <Stack padding={4} paddingTop={0} gap={3}>
            <Text size="sm" color="dim">
              {tool.description}
            </Text>
            <ActionButton
              variant="secondary"
              onClick={() =>
                navigate(tool.canonicalPath || `/research/${tool.id}`)
              }
            >
              View Details
            </ActionButton>
...
export default function ResearchAnalytics() {
  ...
  const contactPath =
    routes.find((r) => r.path === "/contact")?.path || "/contact";
    ...
    <Box as="section">
      ...
      <Stack gap={4}>
        <Stack gap={2}>
          ...
          >
            I build AI-assisted engineering systems that turn messy workflows
            into repeatable software. Grounded DevAI solutions built to ship
            products, not hype.
          </Text>
        ...
        >
          <Stack gap={1}>
            ...
            <Text variant="body" size="lg" color="body">
              Looking for a lead engineer to orchestrate your AI workflows?
            </Text>
          </Stack>
          <ActionButton
            as={NavLink}
        ...
        {/* Mobile Sticky CTA Triggered After Flagship */}
        <Box
        ...
        >
          <ActionButton
            ...
            paddingY={4}
            gap={2}
            className="w-full shadow-glow pointer-events-auto"
          >
            <Icon icon={Send} size="sm" />
        ...
        <Grid cols={{ base: 1, lg: 2 }} gap={12}>
          ...
          >
            <Box
              ...
              width="full"
              height={1}
              className="bg-gradient-to-r from-accent/0 via-accent/40 to-accent/0"
            />
            <Text
              as="h2"
              variant="mono"
            ...
            <Text
              ...
              className="leading-relaxed"
            >
              HRM exposed the need for sophisticated AI orchestration in
              repository operations. By implementing automated PR reviews and CI
              diagnostics, I reduced code review latency by 40% and identified
              blocker patterns in under 5 minutes. RepoAuditor AI was built to
              close this loop, now serving as the backbone for ML engineering
          ...
          >
            ...
            <Text
              ...
              className="leading-relaxed"
            >
              Shipping high-fidelity products requires practical AI
              orchestration, not just hype. My RAG pipelines and automation
              frameworks handle 10k+ telemetry points daily, ensuring that DevAI
              remains a production-grade multiplier rather than an experimental
              novelty. I focus on engineering systems that keep the developer in
              the loop while maintaining high standards.
            </Text>
          </Stack>
        </Grid>
        ...
        {studies.length > 0 && (
          <Stack gap={8} id="articles">
            ...
            <Box display="flex" justify="center" marginTop={4}>
              <ActionButton
                as="a"
