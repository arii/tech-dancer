                  ...
                  <Box className="lg:col-span-4" padding={6}>
                    <Stack gap={0} height="full">
                      ...
                      <Stack direction="col" gap={2.5} marginTop="auto" width="full">
                        {tool.canonicalPath && (
                          <ActionButton as={Link} to={tool.canonicalPath} variant="primary" paddingX={4} paddingY={3} width="full" className="bg-cyan-500 hover:bg-cyan-400 text-black border-cyan-400">
                            OPEN LIVE TOOL →
                          </ActionButton>
                        )}
                        {tool.deepDivePath && (
        ...
        <Stack gap={8} id="experiments-list" marginTop={2} width="full">
          ...
          <Grid cols={{ base: 1, sm: 2, lg: 3 }} gap={6} width="full">
            {experimentTools.map((tool) => (
              <BaseCard key={tool.id} padding={0} gap={0} surface="surface" overflow="hidden">
                <Stack gap={0} height="full">
                  <Box width="full" className="border-b border-white/8 cursor-zoom-in bg-surface-muted bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] opacity-80 min-h-[160px] flex items-center justify-center relative" onClick={() => setLightboxImage(tool.image?.startsWith('/') ? `${baseUrl}${tool.image}` : tool.image || '')}>
                    {tool.image ? (
                      <img
                        src={tool.image.startsWith('/') ? `${baseUrl}${tool.image}` : tool.image}
                        alt={tool.imageAlt || `Preview of ${tool.title}`}
                        className="w-full h-full object-cover absolute inset-0 opacity-heavy hover:opacity-100 transition-opacity duration-500"
                      />
                    ) : (
                       <Icon icon={Activity} size="xl" color="dim" className="opacity-20" />
                    )}
                  </Box>
                  <Stack flex={1} paddingTop={3.5} paddingX={4} paddingBottom={4} gap={0}>
                    ...
                    <Stack direction="row" gap={2} marginTop="auto" width="full" wrap="wrap">
                      {tool.canonicalPath && (
                        <ActionButton as={Link} to={tool.canonicalPath} variant="primary" paddingX={3} paddingY={1.5} width="full">
                          Open Tool <Icon icon={ArrowRight} size="sm" />
                        </ActionButton>
                      )}
                      {tool.deepDivePath && (
                        <ActionButton as={Link} to={tool.deepDivePath} variant="secondary" paddingX={3} paddingY={1.5} width="full">
