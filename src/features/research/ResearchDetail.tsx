...
export default function ResearchDetail() {
  ...
  if (!tool && !study && studies.length > 0) {
    return (
      <Box padding="panel" textAlign="center">
        <Stack gap={8} align="center">
          <Search className="w-12 h-12 opacity-20" />
          <Text variant="display" size="2xl">Item Not Found</Text>
          ...
          <Stack gap={12}>
            {tool ? (
              ...
              <Stack gap={12}>
                <Stack gap={4}>
                  <Text variant="mono" color="brand" size="xs" weight="font-bold" uppercase tracking="widest">
                    RESEARCH_STUDY // {study.category.toUpperCase()}
                  </Text>
                  <Text variant="headline" size="fluid-7">{study.title}</Text>
                  <Box border surface="accent" padding="compact" opacity={5} className="bg-accent/5">
                    <Text variant="body" size="lg" color="body">{study.excerpt}</Text>
                  </Box>
                </Stack>
