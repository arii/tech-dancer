...
*Metadata: ${formData.seoTerms}*`;
  ...
  return (
    <Box as="section" panel>
      ...
      <Box display="flex" direction={{ base: "col", md: "row" }} align={{ md: "end" }} justify="between" gap={8} marginBottom={16} border="b" paddingBottom={12}>
        <Stack gap={6}>
            <Stack direction="row" align="center" gap={3} color="brand" marginBottom={2}>
              <TerminalIcon className="w-5 h-5" />
              <Text variant="mono" tracking="widest" weight="font-bold">Internal_Systems_Console</Text>
            </Stack>
            <Text as="h1" variant="headline" size={{ base: "5xl", md: "8xl" }}>
              The Hub.
            </Text>
            <Text variant="body" size={{ base: "lg", md: "xl" }}>
          ...
          <Box display="flex" surface="dim" padding={1} gap={1} width={{ base: "full", md: "auto" }}>
            <button 
      ...
      <AnimatePresence mode="wait">
        {activeTool === 'drafter' ? (
          <Box 
            ...
            exit={{ opacity: 0, x: 20 }}
            display="grid"
            className="grid-cols-1 md:grid-cols-12 gap-12 max-w-7xl"
          >
            {/* Input Form */}
            <Stack md={{ span: 7 }} gap={8}>
              <Box border surface padding={8}>
                <Stack gap={6}>
                  ...
                  <Grid cols={2} gap={6}>
                    <Stack gap={2}>
                      <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Title</Text>
                      <motion.input 
                    ...
                    <Stack gap={2}>
                      <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Category</Text>
                      <motion.input 
                        whileFocus={{ scale: 1.01 }}
                        type="text" 
                        name="category"
                  ...
                  <Stack gap={2}>
                    <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Tagline / Hook</Text>
                    <motion.input 
                      whileFocus={{ scale: 1.01 }}
                      type="text" 
                      name="tagline"
                  ...
                  <Stack gap={2}>
                    <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Short Excerpt</Text>
                    <motion.textarea 
                      whileFocus={{ scale: 1.01 }}
                      rows={2}
                      name="description"
                  ...
                  <Grid cols={2} gap={6}>
                    <Stack gap={2}>
                      <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">SEO Keywords</Text>
                      <motion.input 
                        whileFocus={{ scale: 1.01 }}
                        type="text" 
                        name="seoTerms"
                    ...
                    <Stack gap={2}>
                      <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Tags</Text>
                      <motion.input 
                        whileFocus={{ scale: 1.01 }}
                        type="text" 
                        name="tags"
                  ...
                  <AnimatePresence mode="popLayout">
                    {formData.type === 'resource' && (
                      <Box as={motion.div}
                        ...
                        overflow="hidden"
                      >
                        <Grid cols={2} gap={6} paddingTop={4} border="t">
                          <Stack gap={2}>
                            <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Product Name</Text>
                            <input 
                              type="text" 
                              name="productName"
                          ...
                          <Stack gap={2}>
                            <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Affiliate Link</Text>
                            <input 
                              type="url" 
                              name="itemLink"
                  ...
                  <Stack gap={2}>
                    <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Featured Image URL / Seed</Text>
                    <Box display="flex" gap={4}>
                      <motion.input 
                        ...
                        className={cn(inputs.base, "flex-1")}
                      />
                      <Box width={12} height={12} surface="dim" border overflow="hidden" shrink={0}>
                        <img 
                          src={formData.image} 
                          alt="Preview" 
                  ...
                  <Stack gap={2}>
                    <Box display="flex" justify="between" align="center">
                      <Text as="label" variant="mono" size="micro" weight="font-bold" color="dim" className="uppercase tracking-widest">Content / Commentary (Markdown)</Text>
                      <Text variant="mono" weight="font-bold" size="micro" color={formData.commentary.length >= 2800 ? "brand" : "dim"} className="uppercase">
                    ...
                    <textarea 
                      ...
                      className={cn(inputs.base, "font-mono", formData.commentary.length >= 3000 && "border-accent")}
                    />
                  </Stack>
                </Stack>
              </Box>
            ...
            <Stack md={{ span: 5 }} gap={8} position="relative" className="md:sticky md:top-0 h-fit">
              <Box display="flex" align="center" justify="between">
                <Text variant="mono" size="micro" weight="font-bold" tracking="widest" color="main">// OUTPUT_BUFFER</Text>
                    ...
                    <AnimatePresence>
                      {copied && (
                        <Box
                          ...
                          exit={{ opacity: 0, y: -10 }}
                          position="absolute"
                          className="-top-10 left-1/2 -translate-x-1/2 bg-accent-brand text-white text-[10px] py-1 px-2 font-mono whitespace-nowrap"
                        >
                          COPIED_TO_CLIPBOARD
                        </Box>
                      )}
              ...
              <Box 
                ...
                shadow="industrial"
              >
                <Box surface="dim" paddingX={6} paddingY={3} border="b" display="flex" align="center" justify="between">
                ...
                <Box as="pre" padding={6} className="text-xs text-text-body font-mono overflow-x-auto whitespace-pre-wrap max-h-[500px] custom-scrollbar">
                  {markdown}
                </Box>
              </Box>
              ...
              <Stack gap={4}>
                ...
                <Text variant="mono" color="dim" align="center" paddingX={8} className="leading-relaxed">
          ...
          >
            <Grid cols={1} md={2} lg={4} gap={6}>
              {[
                ...
                <Box key={stat.label} border surface padding={6} className="scanline-hover">
                  <Stack gap={4}>
                    <Box display="flex" align="center" justify="between" color="dim">
                      <stat.icon className="w-4 h-4" />
                      <Text variant="mono" size="micro" color="accent" weight="font-bold">{stat.trend}</Text>
                    </Box>
                    <Stack gap={1}>
                      <Text variant="headline" size="2xl" color="main" weight="font-black">{stat.value}</Text>
            ...
            <Grid cols={1} md={12} gap={8}>
              <Box border surface padding={8} md={{ span: 7 }}>
                <Stack gap={8}>
                  <Box display="flex" align="center" justify="between">
                    <Text variant="mono" size="micro" weight="font-bold" tracking="wide" color="main">// AUDIENCE_TRAFFIC_WAVE</Text>
              ...
              <Box border surface padding={8} md={{ span: 5 }}>
                <Stack gap={8}>
                  <Box display="flex" align="center" justify="between">
                    <Text variant="mono" size="micro" weight="font-bold" tracking="wide" color="main">// LOAD_LATENCY</Text>
                  ...
                  <Box paddingTop={8} border="t">
                     <Text variant="mono" size="micro" weight="font-bold" tracking="wide" color="main" marginBottom={6}>// SUBSYSTEM_REPORT</Text>
                     <Stack gap={4}>
                       {[
                         ...
                         <Stack key={item.name} gap={1}>
                           <Box display="flex" justify="between" className="text-[8px] font-mono uppercase tracking-widest text-text-dim">
                             <span>{item.name}</span>
                             <span>{item.status}</span>
                           </Box>
                           <Box height={1} surface="dim" width="full" overflow="hidden">
                             <Box 
                              as={motion.div}
                              initial={{ width: 0 }}
            ...
            <Box border surface padding={8}>
              <Text variant="mono" size="micro" weight="font-bold" tracking="wide" color="main" marginBottom={8}>// LIVE_SYSTEM_LOGS</Text>
              <Stack gap={3} className="font-mono text-[10px] uppercase">
                <Stack direction="row" gap={4} color="dim">
                  <Text variant="mono" color="accent" weight="font-bold">[15:32:01]</Text>
                  <Text as="span">Signal_Gateway: Authenticated admin_ari_anders</Text>
                </Stack>
                <Stack direction="row" gap={4} color="dim">
                  <Text variant="mono" color="accent" weight="font-bold">[15:31:45]</Text>
                  <Text as="span">Cache_Purge: Success // TTL reset for site_assets</Text>
                </Stack>
                <Stack direction="row" gap={4} color="dim">
                  <Text variant="mono" color="accent" weight="font-bold">[15:30:12]</Text>
                  <Text as="span">Inbound_Payload: Drafter_Buffer updated via AI_Module</Text>
                </Stack>
                <Box color="dim" opacity="40" className="italic">
                  <Text as="span">[Listening for system events...]</Text>
                </Box>
              </Stack>
            </Box>
          </motion.div>
