...
export const WCSNavigatorPage: React.FC = () => {
  ...
  const handleGenerateItinerary = (_answers: Record<string, QuestionAnswerValue>) => {
    // If not mock mode we would hit an API, but for now we just use the generated trace
    const mockData = MOCK_EVENT_RESULTS[activeEventId] || createGenericMockResult(activeEventName);

    // We update this here just in case they've changed since discovery
    setDecisionTrace(mockData.decisionTrace);

    // We add some visual markdown representation based on what is selected if in demo data
    if(discoveryData?.preset_name) {
      setDiscoveryData(prev => ({...prev, visualScheduleMarkdown: `# Your WCS Visual Schedule for ${discoveryData.preset_name}\n\nGenerated by WCS Navigator.\n\n${mockData.decisionTrace.sessions.map(s => `- ${s.time}: ${s.title}`).join('\n')}`}));
    }
    setStep('results');
  };
      ...
      <Stack gap={8} width="full">
        ...
        {step === 'results' && (
          <Stack gap={6} width="full">
            ...
            <AgentMindTrace
              trace={decisionTrace}
              visualScheduleMarkdown={(discoveryData as any)?.visualScheduleMarkdown || (discoveryData?.preset_name ? `# Your WCS Visual Schedule for ${discoveryData.preset_name}\n\nGenerated by WCS Navigator.` : undefined)}
            />
          </Stack>
        )}
