              ...
              {recentPosts.map((post) => (
                <Box key={post.slug} paddingY={4} as={motion.div} variants={motionTokens.staggerItem}>
                  <Box
                    as={NavLink}
                    to={`/blog/${post.slug}`}
                    className="group transition-colors hover:bg-surface/50" display="flex" direction={{ base: "col", sm: "row" }} gap={{ base: 3, sm: 4 }} radius="lg" paddingX={{ base: 3, sm: 5 }} paddingY={{ base: 5, sm: 6 }} marginX={{ sm: -2 }} align={{ sm: "start" }}
                  >
                    <Box display="flex" shrink={0} className="sm:w-44" wrap align="center" gap={{ base: 2, sm: 3 }} paddingTop={0.5}>
                    ...
                    <Box>
                      <Text as="h3" color="main" size="base" weight="font-bold" className="transition-colors group-hover:text-accent leading-snug" marginBottom={1}>
                        {post.title}
                      </Text>
