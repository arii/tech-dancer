...
export function LatestPosts() {
  ...
  return (
    <Box as="section">
      <Box display="flex" align={{ base: 'start', sm: 'center' }} justify="between" gap={2} marginBottom={6}>
        <Text as="h2" variant="headline" size="2xl" weight="font-black">
          Latest from BoomTick
        </Text>
        <Text as={NavLink} to="/blog" variant="mono" size="xs" color="dim" weight="font-bold" className="uppercase tracking-widest transition-colors hover:text-accent">
          View all →
        </Text>
      </Box>
      ...
      <Stack gap={0} border="t" className="border-line">
        {posts.map((post) => (
          <Box key={post.slug} as={NavLink} to={`/blog/${post.slug}`} display="flex" align="center" gap={4} paddingY={5} border="b" className="group border-line transition-colors hover:bg-surface/50">
            <Box width={18} height={18} radius="md" overflow="hidden" className="shrink-0 bg-surface-alt">
            ...
            <Stack gap={1} flex className="min-w-0">
              ...
              <Text variant="body" size="base" weight="font-bold" className="line-clamp-2 transition-colors group-hover:text-accent">{post.title}</Text>
