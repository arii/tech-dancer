...
export function GearShelf() {
  return (
    <Box as="section">
      ...
      <Box display={{ base: 'none', lg: 'grid' }} cols={2} gap={3}>
        {PICKS.map(({ label, image, href }) => (
          ...
          >
            <Box width={12} height={12} radius="md" overflow="hidden" className="shrink-0 bg-surface-alt">
              <img src={`${ASSET_PREFIX}${image}`} alt={label} className="h-full w-full object-cover object-center" />
            </Box>
      ...
      <Box display={{ base: 'block', lg: 'none' }} overflowX="auto" paddingBottom={2}>
