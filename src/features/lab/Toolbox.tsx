...
export default function Toolbox() {
  ...
  return (
    <Box as="section" paddingY={8}>
      ...
      <Box as="header" border="b" paddingBottom={12} marginBottom={12} className="border-line/50">
        <Box marginBottom={4}>
        ...
        <Box marginBottom={4}>
          <Text as="h1" variant="display" size="4xl" weight="font-black" className="text-accent-navy block">
            Gear Reviews
          </Text>
        </Box>
        <Box marginBottom={8}>
          <Text as="p" variant="sans" size="lg" color="dim" className="max-w-2xl font-medium block">
        ...
        <Box display="flex" align="center" justify="between" gap={4} wrap>
          <Box className="relative w-full max-w-md flex-1">
            <Box
              as="input"
              type="text"
              placeholder="Search gear (e.g. earplugs, shoes)..."
              paddingLeft={10} paddingRight={4} paddingY={3}
              surface="default"
              border
              radius="none"
              className="w-full focus:ring-4 focus:ring-accent/10 outline-none transition-all text-base md:text-sm"
