...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "solid", intent = "default", size = "md", fullWidth, loading, compact, children, ...props }, ref) => {
    return (
      <Box
        as="button"
        ref={ref as any}
        cursor="pointer"
        className={cn(
          "inline-flex items-center justify-center transition-all duration-200 font-bold uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed",
          !compact && "min-h-[44px] min-w-[44px]",
          variant === "primary" && "font-mono tracking-widest text-[10px] px-8",
