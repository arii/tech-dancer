import{A as Ye}from"./index-Bn4bWTM2.js";const mn=`---
type: post
draft: true
status: draft
...
`),E=y!==-1,q=t.inFlow??e.flow??e.items.length===0;if(E||!q){let J=!1;if(E&&(_==="&"||_==="!")){let P=S.indexOf(" ");_==="&"&&P!==-1&&P<y&&S[P+1]==="!"&&(P=S.indexOf(" ",P+1)),(P===-1||y<P)&&(J=!0)}J||(A=`
${t.indent}`)}}else(S===""||S[0]===`
...
`+E.comment:y.comment=E.comment);const J=new j(y,q);if(t.options.keepSourceTokens&&(J.srcToken=b),a){const P=c;ln(t,P.items,y)&&i(_,"DUPLICATE_KEY","Map keys must be unique"),P.items.push(J)}else{const P=new B(t.schema);P.flow=!0,P.items.push(J);const bt=(q??y).range;P.range=[y.range[0],bt[1],bt[2]],c.items.push(P)}u=q?q.range[2]:E.end}}const h=a?"}":"]",[g,...m]=s.end;let f=u;if(g?.source===h)f=g.offset+g.source.length;else{const p=r[0].toUpperCase()+r.substring(1),b=d?`${p} must end with a ${h}`:`${p} in block collection must be sufficiently indented and end with a ${h}`;i(u,d?"MISSING_CHAR":"BAD_INDENT",b),g&&g.source.length!==1&&m.unshift(g)}if(m.length>0){const p=ke(m,f,t.options.strict,i);p.comment&&(c.comment?c.comment+=`
`+p.comment:c.comment=p.comment),c.range=[s.offset,f,p.offset]}else c.range=[s.offset,f,f];return c}function ze(n,e,t,s,i,o){const a=t.type==="block-map"?Fs(n,e,t,s,o):t.type==="block-seq"?Ws(n,e,t,s,o):qs(n,e,t,s,o),r=a.constructor;return i==="!"||i===r.tagName?(a.tag=r.tagName,a):(i&&(a.tag=i),a)}function Us(n,e,t,s,i){const o=s.tag,a=o?e.directives.tagName(o.source,h=>i(o,"TAG_RESOLVE_FAILED",h)):null;if(t.type==="block-seq"){const{anchor:h,newlineAfterProp:g}=s,m=h&&o?h.offset>o.offset?h:o:h??o;m&&(!g||g.offset<m.offset)&&i(m,"MISSING_CHAR","Missing newline after block sequence props")}const r=t.type==="block-map"?"map":t.type==="block-seq"?"seq":t.start.source==="{"?"map":"seq";if(!o||!a||a==="!"||a===B.tagName&&r==="map"||a===V.tagName&&r==="seq")return ze(n,e,t,i,a);let l=e.schema.tags.find(h=>h.tag===a&&h.collection===r);if(!l){const h=e.schema.knownTags[a];if(h?.collection===r)e.schema.tags.push(Object.assign({},h,{default:!1})),l=h;else return h?i(o,"BAD_COLLECTION_TYPE",`${h.tag} used for ${r} collection, but expects ${h.collection??"scalar"}`,!0):i(o,"TAG_RESOLVE_FAILED",`Unresolved tag: ${a}`,!0),ze(n,e,t,i,a)}const c=ze(n,e,t,i,a,l),d=l.resolve?.(c,h=>i(o,"TAG_RESOLVE_FAILED",h),e.options)??c,u=O(d)?d:new T(d);return u.range=c.range,u.tag=a,l?.format&&(u.format=l.format),u}function Ks(n,e,t){const s=e.offset,i=Hs(e,n.options.strict,t);if(!i)return{value:"",type:null,comment:"",range:[s,s,s]};const o=i.mode===">"?T.BLOCK_FOLDED:T.BLOCK_LITERAL,a=e.source?Gs(e.source):[];let r=a.length;for(let f=a.length-1;f>=0;--f){const p=a[f][1];if(p===""||p==="\r")r=f;else break}if(r===0){const f=i.chomp==="+"&&a.length>0?`
